...
const BASE_URL_IND = "https://api.covid19india.org/";
...
const makeSelectOption = async (select, selectedCountry) => {
    ...
    wwOptionDom.dataset.image = location.join('/') + '/../static/icons/global.png';
...
/**
 * Function for Cases by State
 * @returns {Promise<void>}
 */

const casesByState = async () => {
    const data = await getData(BASE_URL_IND + 'state_district_wise.json');
    let noListsShow = 11;
    const stateLists = document.querySelector('.cases-state-lists');
